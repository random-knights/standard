...
const PANEL_STATES = [SAFE, UNCERTAIN, BEYOND, UNKNOWN];
/**
...
function readPanel(raw) {
    ...
    return raw.map((e, index) => {
        ...
        return {
            ...
            hasTransgressedKey: o.transgressed !== null && o.transgressed !== undefined,
 ...
 * invented severity.
 */
function stateFromValue(value, boundary, highRisk, direction) {
    if (direction !== "benefit" && direction !== "burden")
        return null;
    const past = direction === "benefit" ? value < boundary : value > boundary;
    if (!past)
        return { state: SAFE, transgressed: false };
    if (highRisk === null)
        return { state: UNCERTAIN, transgressed: true };
    const beyond = direction === "benefit" ? value < highRisk : value > highRisk;
...
function verifyPublishedScoreDoc(raw, options = {}) {
    ...
    const boundariesRaw = (doc.boundaries ?? null);
    ...
    if (boundariesRaw === null) {
        ...
        const published = new Set(panel.map((e) => e.id).filter((id) => id !== null));
        const missing = NINE_BOUNDARY_IDS.filter((id) => !published.has(id));
        if (panel.length !== NINE_BOUNDARY_IDS.length || missing.length > 0) {
            fail("boundaries.panel", panel.length, NINE_BOUNDARY_IDS.length, missing.length === 0
        ...
        for (const e of panel) {
            ...
            if (e.controlVariableCitation === null || e.controlVariableCitation === "") {
                fail(`${where}.controlVariableCitation`, "(absent)", "a citation", "no boundary appears in the panel without a citation for its " +
                    "control variable (section 6, condition 2)");
            }
            if (e.thresholdBoundary !== null &&
                (e.thresholdCitation === null || e.thresholdCitation === "")) {
                fail(`${where}.threshold.citation`, "(absent)", "a citation", "an entry publishing a numeric threshold cites the source for it " +
            ...
            if ((e.state === UNKNOWN) !== unknownByData) {
                fail(`${where}.state`, e.state, unknownByData ? UNKNOWN : "a computed state", "state is unknown if and only if the value and transgressed are " +
            ...
            if (e.state === UNKNOWN) {
                ...
                if (e.evaluationNote === null || e.evaluationNote === "") {
                    fail(`${where}.evaluationNote`, "(absent)", "a reason", "an unknown entry says why it could not be evaluated, so that an " +
            ...
            if (e.valueDomainId !== null && NON_CONTROL_DOMAINS.has(e.valueDomainId)) {
                fail(`${where}.value.domainId`, e.valueDomainId, "a domain whose indicator IS this boundary's control variable", "only a domain whose indicator is the accepted control variable may " +
            ...
            if (e.controlValue !== null && e.thresholdBoundary !== null) {
                const expect = stateFromValue(e.controlValue, e.thresholdBoundary, e.thresholdHighRisk, e.thresholdDirection);
                if (expect === null) {
                    fail(`${where}.threshold.direction`, e.thresholdDirection, "benefit | burden", "the threshold does not say which side of the boundary is safer, " +
                        "so the state cannot be recomputed from the value");
                }
                else {
                    checked += 2;
                    if (e.state !== expect.state) {
                        fail(`${where}.state`, e.state, expect.state, "state is computed from the published control VALUE against the " +
                    ...
                    if (e.transgressed !== expect.transgressed) {
                        fail(`${where}.transgressed`, e.transgressed, expect.transgressed, "transgressed does not follow from the published value and " +
            ...
            if (e.hasValue && e.controlValue !== null) {
                if (e.valueProvenance === null || e.valueProvenance === "") {
                    fail(`${where}.value.provenance`, "(absent)", "a provenance rung", "an evaluated entry carries the provenance of the input it was " +
                        "evaluated from (section 6)");
                }
                else if (!NON_PROVISIONAL_RUNGS.has(e.valueProvenance)) {
                    if (e.provisional !== true) {
                        fail(`${where}.value.provisional`, e.provisional, true, "an entry evaluated from a " +
                            e.valueProvenance +
        ...
        if (breachCountPublished === null) {
            ...
            if (!same(breachCountPublished, recomputedBreaches)) {
                ...
                const framework = (boundariesRaw.framework ?? null);
        ...
        if (unknownCount !== null && !same(unknownCount, recomputedUnknown)) {
            fail("boundaries.unknownCount", unknownCount, recomputedUnknown, "unknownCount is the count of entries with state unknown");
        }
        if (evaluatedCount !== null &&
            !same(evaluatedCount, panel.length - recomputedUnknown)) {
            fail("boundaries.evaluatedCount", evaluatedCount, panel.length - recomputedUnknown, "evaluatedCount is the count of entries that are not unknown, which " +
        ...
        if (totalBoundaries !== null &&
            ...
            fail("boundaries.totalBoundaries", totalBoundaries, evaluatedCount + unknownCount, "evaluatedCount plus unknownCount equals totalBoundaries, or a " +
