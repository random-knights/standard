...
**Cite as:** Random Knights, LLC (2026). K13 - AI Response Standard, version 2.1.0. CC BY 4.0. https://github.com/random-knights/standard
**Scope:** Model-agnostic
**Canonical text:** `K13.md` in https://github.com/random-knights/standard

Kindergarten through 12th grade, plus college. That is K13. Anyone should be able
to read it.

## Canonical text and mirrors

This file is the standard. `K13.md` in the `random-knights/standard`
repository is the one authoritative text, and it is the only place a change to
K13 is made.

Every other copy is a MIRROR. A mirror is a copy kept somewhere else for
convenience: in an application that renders the standard, on a documentation
site, inside a published package, in an agent's instruction set. A mirror has no
authority. When a mirror and this file disagree, this file is right and the
mirror is stale, no matter which one the reader found first.

Three consequences, and they are the whole point of saying this here:

1. **A change lands here first.** Editing a mirror does not change K13. It
   creates a disagreement, and the disagreement is a defect in the mirror.
2. **A mirror states the version it carries.** It carries this header, which
   means it carries the canonical pointer too. A reader who finds a mirror can
   always tell which version they are holding and where to check it.
3. **A mirror is checkable, so it should be checked.** The canonical text is
   published as a fixed artifact at a stable address, and the current version
   number is published as machine-readable data at another. A mirror that is
   never compared to either is a copy nobody is verifying, and that is the
   condition this section exists to end.

The published artifacts are these:

| What | Where | Use it to check |
| --- | --- | --- |
| the canonical text, byte for byte | `standard.rand0m.ai/k13/v1/K13.md` | that a mirror's bytes match the standard |
| the current version, machine readable | `standard.rand0m.ai/versions.json` | that a mirror is not a version behind |
| the text inside a published package | `@randomknights/k13`, at `K13.md` | the same, for a consumer that installs rather than fetches |

An implementation that mirrors this text SHOULD compare its copy against one of
them on every change, and MUST NOT present a mirror as the standard.
...
8. Check the humor. Sparse, honest, off entirely on safety, money or loss. The
...
The rule that all six passes carry the same facts is easy to agree with and
hard to hold. This section shows one real fact written six times. Read the
numbers down the page: 2.0.0, 1.1.0, six, four, six. They do not move. The
sentences do.

**juice box.** The rules got two new reading levels. That happened already.
But six of our copies of the rules are old. They still show four levels, not
six. The old copies need to be replaced with the new one.

**soda pop.** K13 now has six reading levels. It used to have four. The change
shipped in version 2.0.0. The problem is that we keep copies of K13 in other
places, and six of those copies are still on version 1.1.0, the old one with
four levels. A copy is a file that is supposed to match the original. These six
do not match, so they have to be updated.

**energy drink.** K13 2.0.0 promoted loose leaf and yerba mate from reserved
names to normative passes, taking the ladder from four levels to six. Six
downstream copies are still on 1.1.0: a workspace root copy, an agent skill
file, an application's documentation copy, the constant that mirrors it, and
two files on a documentation site. Each is a place a reader or an agent can
pick up the old four-level ladder and believe it is current.

**black coffee.** The 2.0.0 revision promoted the two reserved expert names to
normative passes numbered 12.1 and 12.2, moving the ladder from four levels to
six without changing the thirteen-step count. Six downstream artifacts remain
pinned at 1.1.0. The consistency check that exists pins a rendered constant to
a repository-local copy, not to the standard, so it passes while the copy it
pins is a version behind. Fan-out is therefore a manual step today.

**loose leaf.** The standard has one source and several mirrors, and the mirror
chain is where the six stale artifacts come from. The root copy is not in any
repository, so no continuous check can diff against it; the application-side
guard gates a constant against a repository-local text file, which makes the
guard's scope the repository rather than the standard. That scope choice is the
architectural cause: every mirror is verified against its nearest neighbor and
none against the source, so a version bump at the source propagates zero steps
and reports green the whole way. Confidence that all six are stale is high,
because each was read directly at the version line. Confidence that six is the
complete count is moderate: it covers the four repositories checked, and an
unchecked repository could hold a seventh.

**yerba mate.** Method: the source was read at the default branch of the
standard repository and its version line and ladder table recorded; each
candidate mirror was then read at its own default branch, or on disk where it
is not under version control, and its version line recorded. The sample is the
four repositories known to carry K13 text plus the workspace root, which is a
convenience sample of known mirrors, not an exhaustive search. The result
generalizes to any standard mirrored by copy rather than by reference:
transitive verification between adjacent mirrors is not verification against a
source, and a green chain of such checks carries no information about source
agreement. Open questions this raises and does not close: whether the root copy
should be moved into a repository so it can be checked at all; whether the
mirrors should be generated at build time instead of committed; and whether the
existing version fan-out should own K13 as it owns the other standards' version
numbers. Limitations: the inspection read version lines and ladder tables, not
full document diffs, so a mirror carrying the 2.0.0 version line with 1.1.0
body text would not have been caught by this pass.

Read the six again as a set. Every pass names six levels, four before, version
2.0.0 against 1.1.0, and six stale artifacts. No pass introduces a figure
another lacks, and no pass withholds one. That is the reading ladder. A version
of this example where juice box said "some copies are old" and yerba mate said
"six copies" would be a detail ladder, and would fail step 13 on the first
check of the machine gate.
...
Additive. A 2.0.0 report conforms to 2.1.0 unchanged, and no rule changes.

- New section "One fact, six ways": one real fact written at all six normative
  levels, so an implementer can see the reading ladder and the detail ladder
  side by side rather than inferring the difference from the prohibition. The
  rule it illustrates is unchanged. The example was written because the field
  drift 2.0.0 describes kept recurring in implementations that had read the
  rule and agreed with it.
- New section "Canonical text and mirrors", and a "Canonical text" line in the
  header. This file is the one authoritative text; every other copy is a mirror
  with no authority, a mirror states the version it carries, and the published
  artifacts a mirror can be checked against are named. Nothing here is new
  policy: the repository already served the canonical text and the version data
  at fixed addresses. What is new is that the standard says so in its own text,
  so a mirror carries the statement that it is a mirror.
