...
4. The schema (`aieds.schema.json`) is licensed Apache 2.0 (`LICENSE`); methodology content (`methodology.md`) and the coefficient tables are CC BY 4.0 (`LICENSE-DOCS`). Forks may adapt the methodology but must attribute and use a distinct version prefix.
